...
export function Editor({ onBack }) {
  const { invoice, updateSettings, updateClient, addItem, updateItem, removeItem, totals, formatCurrency, saveDraft, saveInvoiceFinal, companySettings, savedClients, selectClient } = useInvoice();
  ...
  const handleSaveFinal = async () => {
    const success = await saveInvoiceFinal();
    setShowSaveDialog(false);
    if (success) {
          ...
          <div className="flex gap-2 flex-wrap justify-end items-center">
             {!isSaved ? (
               ...
               <>
                 ...
                 <GlassButton onClick={handlePrint} variant="secondary" className="flex items-center gap-1.5 text-xs">
